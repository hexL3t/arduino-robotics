...
void loop() {
  ...
  hexagon();             // Perform the hexagon pattern
  stopMotors();           // Stop the motors
}

// Hexagon function - makes the robot move in a hexagon pattern
void hexagon(){
  for(int count = 0; count < 6; count++){  // Repeat the following 6 times to form a hexagon
    forward(100);           // Move forward at speed 100
    delay(1000);            // Wait for 1 second
    spinLeft(100);          // Spin left at speed 100
    delay(275);             // Wait for 0.275 seconds (spin duration)
  }
}

// Pentagon function - makes the robot move in a pentagon pattern
void pentagon(){
  for(int count = 0; count < 5; count++){  // Repeat the following 5 times to form a pentagon
    forward(100);           // Move forward at speed 100
    delay(1000);            // Wait for 1 second
    spinLeft(100);          // Spin left at speed 100
    delay(330);             // Wait for 0.33 seconds (spin duration)
  }
}

// Triangle function - makes the robot move in a triangle pattern
void triangle(){
  for(int count = 0; count < 3; count++){  // Repeat the following 3 times to form a triangle
    forward(100);           // Move forward at speed 100
    delay(1000);            // Wait for 1 second
    spinLeft(100);          // Spin left at speed 100
    delay(550);             // Wait for 0.55 seconds (spin duration)
  }
}

// Square function - makes the robot move in a square pattern
void square(){
  for(int count = 0; count < 4; count++){  // Repeat the following 4 times to form a square
    forward(100);           // Move forward at speed 100
    delay(1000);            // Wait for 1 second
    spinLeft(100);          // Spin left at speed 100
    delay(400);             // Wait for 0.4 seconds (spin duration)
  }
}
