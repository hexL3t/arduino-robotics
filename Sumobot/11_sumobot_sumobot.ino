...
void setup() {
  ...
  pinMode(A1, OUTPUT);   // Set Pin A1 as an output for ultrasonic sensor trigger
  pinMode(A0, INPUT);    // Set Pin A0 as an input for ultrasonic sensor echo
...
void sumoBot(){
  if ((digitalRead(6) === 1) && (digitalRead(8 ===1)) {
    measuredDistance();
    if (distance > 30) {
      spinLeft(100);
    }
...
void measuredDistance() {
  ...
  delayMicroseconds(10);                          // Delay for 2 microseconds
  digitalWrite(A1, LOW);                          // Send a low pulse to the trigger pin
...
void edgeFollowingRobot() {
  if (digitalRead(8) == 0) {                                    // When sensor on Pin 8 detects black (0)
    forwardTurn(40, 80);                                        // The slower speed goes to the inside wheel
  }
  else {
    forwardTurn(80, 40);                                        // Swap Speeds when off the line
  }
}

void driveInsideCircle() {
  if(digitalRead(8) == 0 && digitalRead(6) == 0){
    forward(100);
  }
  else {
    reverse(100);
...
void stopOnWhite() {
  while (!(digitalRead(6) == 0) && (digitalRead(8) == 0)){      // Loop until not both pins 8 and 6 are reading black
    forward(100);                                               // Move forward at full speed
  }
  stopMotors();                                                 // Stop motors when both sensors detect black
}
