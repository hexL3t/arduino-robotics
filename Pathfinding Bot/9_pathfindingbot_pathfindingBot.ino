...
void pathfindingBot(int distance) {
  lcd.clear();
  count += 10 * direction;          // Increment/decrement servo angle

  // Check if servo angle is within valid range
  if (count >= 0 || count <= 180) {
    myServo.write(count);           // Move servo to the current angle
    lcd.setCursor(0, 0);
    lcd.print("Checking...");

    lcd.clear();
    if (distance > 20) {            // Path is clear
      lcd.setCursor(0, 0);
  ...
  // Reverse direction when servo reaches end of range
